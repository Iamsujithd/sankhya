...
def get_chat_model(model_type=4, user_api_key=None):
    user_api_key = api_key if user_api_key is None or user_api_key == "" else user_api_key
    if user_api_key and user_api_key.startswith("gsk_"):
        if model_type in [4, "4", "GPT-4-Turbo"]:
            model_name = "llama-3.3-70b-versatile"
        elif model_type in [3.5, "3.5", "GPT-3.5-Turbo"]:
            model_name = "llama-3.1-8b-instant"
        else:
            model_name = model_type if isinstance(model_type, str) else "llama-3.3-70b-versatile"
        return ChatOpenAI(
        ...
        if model_type in [4, "4", "GPT-4-Turbo"]:
            ...
            model_name = model_type if isinstance(model_type, str) else model4_name
        return ChatOpenAI(
            model_name=model_name,
            openai_api_key=user_api_key,
            temperature=0
...
def decide_encode_type(attributes, data_frame_head, model_type = 4, user_api_key = None):
    ...
    """
    try:
        llm = get_chat_model(model_type, user_api_key)
